...
create table Destination(	ID_Dest number primary key,Nom_Destination varchar2(20) not null ,
				Pays varchar2(20) not null) tablespace ts0;


create table Circuit(	ID_Circuit number primary key,
...
create table Etape(	ID_Etape number ,
			ID_Circuit number ,
			Descriptif varchar(50) default ' ')  tablespace ts0  ;

create table Reservation ( ID_Client number not null, 
			   ID_Hotel number not null,
			   Date_reservation date) tablespace ts0;

create table Hotel(	ID_Hotel number primary key, ID_Etape number not null,
			Nom_Hotel varchar2(20) not null,Adresse varchar2(50) default('adresse inconnue') ,
			ID_Classe number default(1), Capac_S number default(0) CHECK (Capac_S >0), 
			Capac_D number default(0) CHECK (Capac_D >0)  )tablespace ts0 ;		

create table Classe_Hotel(	ID_Classe number primary key, Prix_S float not null CHECK (Prix_S >0.0) , 
...
create table Sejour(	ID_Sejour number primary key, Duree number not null CHECK (Duree >0) , 
			Description varchar2(50) default(' '), 
			Coeff float default (1.0) CHECK (Coeff >0.0) )tablespace ts0 ;					

create table Assoc_Prix_Sejour_Circuit(	ID_Circuit number, 
					ID_Sejour number, 
					Prix float not null  CHECK (Prix >0.0)  )tablespace ts0 ;

--Prix aller/retour
create table Vol(	ID_Vol number primary key, ID_Dest number, 
			Prix_Enfant float CHECK (Prix_Enfant >0.0) , 
			Prix_Adulte float CHECK (Prix_Adulte >0.0) )tablespace ts0 ;

create table Client(	ID_Client number primary key ,Adresse varchar2(50),Tel varchar2(10), 
			Nom varchar2(20) not null,Prenom varchar2(20)not null, 
       	     	    	Age number   CHECK (Age >0 and Age < 150) , Email varchar2(30),
			Classe_Sociale varchar2(20), ID_Dest_Preferee number, 
...
create table Facturation(ID_Facture number not null, Date_Facture date default trunc(sysdate), 
       	     		Adresse_Client varchar2(50) not null, Tel varchar2(10) not null, 
			Nom varchar2(20) not null, Prenom varchar2(20) not null, 
			Nom_Dest varchar2(20)not null, Pays_Dest varchar2(20) not null, 
			Nom_Hotel varchar2(20)not null, Address_Hotel varchar2(50) not null,
			Classe_Hotel number not null,
			Prix_S float not null , Prix_D float not null,
			Nom_circuit varchar2(20)not null, Duree_sejour number not null, 
			...
			Total_Vol float not null , Total_Hotel float not null, 
			Total_Circuit float not null, Total_Facture float not null,
